...
export default {
  ...
  methods: {
    ...
    async getClassInfo(id) {
      this.classId = id;
      const response = await this.$axios.get(`http://10.149.0.206/info/${id}`);
      this.className = response.data.desc;
      this.classInfo = response.data.fields.sort(
        (a, b) => a.name.toLowerCase().localeCompare(b.name.toLowerCase()),
      );
    },

    async getClassesList() {
      const response = await this.$axios.get('http://10.149.0.206/info/list');
